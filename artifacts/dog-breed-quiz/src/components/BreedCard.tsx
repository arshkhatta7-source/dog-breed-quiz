...
export function BreedCard({ breed, score, matchReasons, rank }: BreedCardProps) {
  ...
  return (
    ...
    >
      <Card
        className="overflow-hidden border-2 transition-all hover:shadow-md group"
        data-testid={`breed-card-${breed.id}`}
      >
        <div className="md:flex">
          <div className="relative h-64 md:h-auto md:w-2/5 shrink-0 overflow-hidden bg-muted">
            ...
            {breed.isProtectionBreed && (
              <div className="absolute top-4 right-4 bg-orange-500 text-white px-2.5 py-1 rounded-full text-xs font-bold shadow-sm">
                Protection Breed
              </div>
            )}
          </div>
          ...
          <CardContent className="p-6 md:p-8 flex flex-col justify-between flex-1">
            ...
            <Button
              ...
              data-testid={`breed-card-link-${breed.id}`}
            >
              <Link href={`/breed/${breed.id}`}>Learn more about {breed.name}</Link>
            </Button>
          </CardContent>
        </div>
      </Card>
